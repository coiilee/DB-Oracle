...
IF-ELSE문과 비슷한 역할을 수행, 다양한 상황에 맞게 조건을 검사할 수 있음. 
...
여러 조건을 순서대로 검사하고 , 조건에 맞는 첫번째 결과를 반환 
...
2. CASE 컬럼 비교 문법 (단일값 비교)
단일 컬럼 값이 여러 조건 중 하나와 일치하는지 검사하고, 일반적으로 값이 같은지 비교하는데 사용 
[작성법]
CASE 컬럼명 
    WHEN 값1 


-- ORDER 테이블에 작성된 컬럼내 값으 한번에 변경할 때 사용하는 예제 
