...
DOUBLE              : 실수형 자료형 
DECIMAL OR NUMERIC  : 고정소수점을 표현한 숫자 
...
가변 길이 
VARCHAR     : 변수 공간의 길이를 변경 가능한 문자열
              실제 데이터의 길이만큼 저장함                     최대 255자(숫자,영어 기준) 저장 가능 
              
TEXT        : 긴 글자를 저장할 때 사용                          최대 65,535자까지 저장 가능 
MEDIUMTEXT  : TEXT보다 큰 텍스트를 저장                         최대 16MB 
LONGTEXT    : TEXT, MEDIUMTEXT보다 큰 텍스트를 저장             최대 4GB

기타 자료형
BLOB        : 이미지, 동영상 등과 같은 이진 데이터 저장 
ENUM        : 미리 정의된 값을 선택하는데 사용 (택 1가지 선택)
              HTML에서 SELECT - OPTION   RADIO 처럼 지정된 범위에서만 선택 가능함 
              EX) ENUM('small','medium','large') = 작음 중간 큼 중 하나를 선택해서 db에 저장 
SET         : 여러 값을 가질 수 있는 집합을 정의하는 자료형 (다수 선택 가능)
              CHECKBOX 처럼 다수 선택해서 DB에 저장 
              EX) SET('독서','낚시','쇼핑') = 독서 낚시 쇼핑을 한번에 저장 가능 
...
TIMESTAMP : 날짜와 시간이 함께 저장되는데, 값이 변경될 때 자동으로 현재시간으로 갱신해줌 
