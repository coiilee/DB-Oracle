/*
cmd(명령 프롬포트 창)에서 mysql 데이터베이스 명칭, 유저생성, 권한 설정 접속 
mysql -u root -p
...
--권한 허용 (모든 특권을 허용)       데이터베이스.* = 데이터베이스 내에 있는 모든 테이블 접근 ok    어떤 계정에 권한을 부여하는가
...
-- 주의할점 : 데이터베이스명칭, 유저명, 유저비밀번호는 대소문자 띄어쓰기 모두 구분함 
CREATE DATABASE 데이터베이스명칭;
