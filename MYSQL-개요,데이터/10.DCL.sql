...
사용자 계정 : 데이터 베이스에 대해서 질의, 갱신, 보고서 작성 등의 작업을 수행할 수 있는 계정으로 
              업무에 필요한 최소한의 권한만 가지는 것을 원칙으로 함. 
              ex) newbie(신입)계정 , staff, associate, seniorStaff 등 각 직급에 맞는 권한만 가지는 것 
...
--> 외부 컴퓨터에서 회사 DB에 접근 가능함 

CREATE USER '유저아이디'@'localhotst' IDENTIFIED BY '유저비밀번호';
...
-- 호스트에는            %(모든 외부 컴퓨터), 
--                       localhost(datbase가 존재하는 컴퓨터), 
--                       192.168.0.1(현재 네트워크 내에 존재하는 컴퓨터) : 우리집 거실에 있는 컴퓨터
--                       211.0.0.1(특정 ip주소에서만 접근 가능)          : 개똥이네에서 우리집 db 접속 가능 


-- 계정 OWNER/NEWBIE 
-- 예를들어 KHCAFE / KHUSER 데이터베이스가 존재하고, 
-- 각 직급에 따라 데이터베이스 접속 권한 부여 

GRANT ALL PRIVILEGES ON KHCAFE.menu TO 'NEWBIE'@'%';
--> 새로운 신입 계정은 KHCAFE 데이터베이스에서 menu 테이블만 접근 가능한 것 
...
-- JUNIOR가 이상한 행동을 하는 것 같음. 기존에 부여한 권한 중 INSERT, UPDATE를 회수 
REVOKE INSERT, UPDATE ON KHUSER.user FROM 'JUNIOR'@'%'; 

--JUNIOR가 이상한 행동을 보이는게 맞음! 혹시 모르니 모든 권한 회수 
REVOKE ALL PRIVILEGES ON KHUSER.* FROM 'JUNIOR'@'%'; 

/*
권한 부여 
GRANT ______ TO _______ ;
권한 회수 
REVOKE ______ FROM ______ ; 
*/
