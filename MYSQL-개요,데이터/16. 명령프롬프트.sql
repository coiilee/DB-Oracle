...
특정 데이터 베이스 접속 
C:\Users\user1>mysql -u 유저명 -p데이터베이스명 

-- 데이터베이스 관련 명령어 
SHOW DATABASE;             --> 데이터베이스 목록 확인 
USE 데이터베이스이름;       --> 특정 데이터베이스로 접속 
SHOW TABLES;               --> 특정 데이터베이스 내에 존재하는 테이블 목록 확인 
SHOW COLUMNS FROM 테이블명 --> 특정 테이블 내에 컬럼 구조 확인 

이외 CREATE ~ DELETE COMMIT 등 모든 명령어 사용 가능함 
; 세미콜론 작성하기 전까지 이어서 쿼리 작성 가능 

MYSQL 종료하길 원한다면 EXIT를 작성하거나 오른쪽 위에 있는 X표시 누르면 종료 가능함. 

아래 ↓ 명령 프롬프트에서 MYSQL 사용한 예제. 
