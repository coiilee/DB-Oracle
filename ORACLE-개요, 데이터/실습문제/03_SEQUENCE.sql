...
--  			먹고 가는 사람 수를 100번부터 100,101, 102, ... 순차적으로 먹고간 인원 체크 
--				CREATE SEQUENCE 가져간다 START WITH 700 INCREMENT BY 1; 
-- 				가져가는 사람 수를 700번부터 700, 701, 702, .. 순차적으로 가져간 인원 체크 

CREATE SEQUENCE seq_user; -- START WITH 1 INCREMENT BY 1; 이 기본값으로 설정됨 


-- 테이블 생성 user 예약어 이기 때문에 table 명으로 작성할 경우 ``이나 ""로 만들어야함 
...
-- 트리거 생성 = 데이터 삽입 시 시퀀스를 자동으로 활용할 수 있도록 설정 
-- 트리거를 이용해서 테이블에 새로운 숫자값이 삽입될 때 시퀀스에 저장된 번호를 호출해서 ID값을 자동 설정해주는 것 
-- 무엇을 만들거나 수정하거나 삭제할 때 OR REPLACE 사용하면 새로 만들거나 존재하면 덮어쓰기 가능 
CREATE OR REPLACE TRIGGER trg_std 
BEFORE INSERT ON student 
FOR EACH ROW 
BEGIN 
	-- 시퀀스를 사용해서 STUDENT 테이블 내에 STUDENTID 번호를 자동으로 생성하기 
