...
대표적으로 VARCHAR2 , NUMBER, TIMESTAMP를 많이 사용함 

1. 문자열 타입 (Character Date Types)
CHAR(n)         : 고정 길이 문자열 한글자씩 최대 몇글자 작성할 것인지 설정 / 
                  작성을 안했을 경우 1byte로 컬럼 하나에 오직 [숫자나 영어] 한글자만 가능 

VARCHAR(n)      : Oracle 에서는 사용 불가, 표준 SQL에서는 가능 

**VARCHAR2(n)     : VARCHAR보다 더 큰 공간 사용할 수 있음 
                  필수로 길이를 지정해야함 (평균 255byte로 많이 작성함)
                  Oracle 이외 어디든 사용 가능 
