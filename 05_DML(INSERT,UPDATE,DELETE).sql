...
---데이터를 저장하고 수정하고 삭제한 후 반드시 commit를 작성해야지 저장 
...
-- CAFE 테이블을 복제해서 EATS 테이블을 생성 
CREATE TABLE EATS AS SELECT * FROM CAFE; --CAFE 테이블 형식을 그대로 복제해서 EATS 테이블을 생성함 

/**
CREATE TABLE EATS AS SELECT * FROM CAFE;
카페 테이블 형식을 그대로 복제해서 EATS 테이블을 생성한다는 의미 
CREATE TABLE Cafe (
...
CREATE TABLE EATS (
    ...
    cafe_name VARCHAR2(100) NOT NULL,  -- 이츠 이름
    location VARCHAR2(255),            -- 이츠 위치
    contact_number VARCHAR2(20)        -- 이츠 연락처
);
...
----만약 WHERE 조건을 설정하지 않으면 모두 삭제  
----DELETE의 경우 진행하기 전 주위에 한번 더 점검한 후 진행하세요~~~~~ 
